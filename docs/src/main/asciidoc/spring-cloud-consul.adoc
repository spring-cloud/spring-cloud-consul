...
* `format` The data format in Consul, see <<YAML or Properties with Config>> and <<git2consul with Config>>
* `defaultContext` sets the folder name used by all applications
* `defaultContextFormat` The data format used by the default context, only needs to be specified if the default context format differs from the app context format
...
It may be more convenient to store a blob of properties in YAML or Properties format as opposed to individual key/value pairs.  Set the `spring.cloud.consul.config.format` property to `YAML` or `PROPERTIES`. If a different format is used for the `defaultContext` it can be configured with the `spring.cloud.consul.config.default-context-format` property. For example to use YAML:
