...
==== Instances used by Load-balancer

The load balancer will only use instances of a service with health checks in "passing" state by default. However Consul also offers the "warning" state, which you might still want to accept for routing. You can configure the accepted states via property `spring.cloud.consul.discovery.status-considered-as-healthy`, e.g.
.application.yml
----
spring:
  cloud:
