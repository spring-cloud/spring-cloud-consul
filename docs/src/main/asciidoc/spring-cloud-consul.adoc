...
you use property `spring.cloud.consul.discovery.server-list-query-datacenters.STORES=dc-west` where `STORES` is the service name/id and `dc-west` is the datacenter
