...
and use it like this (notice how we use the STORES serviceId from Consul instead of a fully qualified domainname):
...
public String hi(@RequestParam(value="name", defaultValue="Artaban") String name) {
   return this.restTemplate.getForObject("http://STORES/products/1", String.class);
}
----

If you have Consul clusters in multiple datacenters and you want to access a service in another datacenter a serviceId alone is not enough. In that case
you use property `spring.cloud.consul.discovery.server-list-query-datacenters.STORES=dc-west` where `STORES` is the serviceId and 'dc-west' is the datacenter
